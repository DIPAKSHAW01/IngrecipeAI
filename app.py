...
def normalize_recipe_lists(recipes):
    """Ensure ingredients and instructions are lists, not strings."""
    for recipe in recipes:
        # Handle ingredients
        if isinstance(recipe.get('ingredients'), str):
            # Split by newlines or semicolons, and remove empty strings
        ...
        if isinstance(recipe.get('instructions'), str):
            # Split by newlines and remove numbers/dots at the beginning
            instructions = recipe['instructions'].split('\n')
            instructions = [
                re.sub(r'^\d+\.\s*', '', inst.strip())
...
def getresponse():
    if request.args:
        ...
        # Normalize recipes to ensure ingredients and instructions are lists
        recipes = normalize_recipe_lists(recipes)
